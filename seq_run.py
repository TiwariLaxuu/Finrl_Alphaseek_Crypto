...
class SeqData:
    def __init__(self, args: ConfigData, train_ratio: float = 0.8):
        ...
        print('SEQ Data Loaded', self.train_seq_len, self.valid_seq_len)

    def sample_for_train(self, batch_size: int = 32, seq_len: int = 4096, device: th.device = th.device('cpu')):
        print('self.train_seq_len ', self.train_seq_len, 'seq len ', seq_len, 'batch size ', batch_size)
...
if __name__ == '__main__':
    ...
    train_model(gpu_id=GPU_ID)  # Using advance factor to train recurrent network RNN ​​(LSTM+GRU + Regression)
