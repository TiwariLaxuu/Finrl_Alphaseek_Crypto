...
def train_agent(args: Config):
    ...
    """init agent.last_state"""
    state = env.reset()
    print('satte infro ', state.shape[1], args.state_dim)
    # if args.num_envs == 1:
    #     assert state.shape[1] == args.state_dim
    #     assert isinstance(state, np.ndarray)
    #     state = torch.tensor(state, dtype=torch.float32, device=agent.device).unsqueeze(
    #         0
    #     )
    # else:
    #     print( state.shape, args.num_envs, args.state_dim)
...
def run():
    ...
    env_args = {
        ...
        "num_envs": num_sims,
        "max_step": max_step,
        "state_dim": 101 + 2,  # factor_dim + (position, holding)
